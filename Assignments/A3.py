...
def clean_up_board(l):
    ...
    for i in reversed(playable_board):
...
def play_game(board):
    ...
    while discovered!=board:
        ...
        b=False
        try:
            a=discovered[p1]==board[p1]
            b=discovered[p2]==board[p2]
        except IndexError:
            index_eval=True
        while (len(board)-1)<p1 or p1<0 or (len(board)-1)<p2 or p2<0 or p1==p2 or index_eval or a or b:
            print('Invalid input.\nEnter two distinct positions on the board that you want revealed.\ni.e two integers in the range [1, '+str(len(board))+']')
    ...
    print('Congratulations! You completed the game with '+str(tries)+' guesses. That is '+str(tries-(len(board)//2))+' more than the best possible.')
